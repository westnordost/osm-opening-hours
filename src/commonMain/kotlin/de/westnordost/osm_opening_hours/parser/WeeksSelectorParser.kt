...
internal fun StringWithCursor.parseWeeksSelector(lenient: Boolean): WeeksSelector {
    val start = nextNumberAndAdvance(lenient, 2) ?: fail("Expected a week number")
    ...
    if (nextIsRangeAndAdvance(lenient)) {
        ...
        val end = nextNumberAndAdvance(lenient, 2) ?: fail("Expected an end week number")
        ...
        val step = if (nextIsAndAdvance('/', lenient, skipWhitespaces = true)) {
            skipWhitespaces(lenient)
            nextNumberAndAdvance(lenient)?.toInt() ?: fail("Expected a week interval")
        } else null
